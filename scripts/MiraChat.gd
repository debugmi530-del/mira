...
# Ответы по темам — массивы для случайного выбора
...
func _generate_response(user_text: String) -> String:
	...
	# Кто ты / что ты
	if (lower.contains("кто") or lower.contains("что")) and (lower.contains("ты") or lower.contains("ты такая")):
	...
	# Страшно / боюсь / страх
	if lower.contains("страшн") or lower.contains("боюсь") or lower.contains("страх") or lower.contains("бояться") or lower.contains("scary"):
	...
	# Если много сообщений — иногда ссылается на прошлое
