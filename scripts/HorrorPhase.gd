...
func _run_wall_sequence() -> void:
	...
	for wall in walls:
		...
		wb.free()
		_queue_text(lines, TYPE_SPEED)
		await get_tree().create_timer(LINE_GAP * lines.size() + 2.0).timeout
	...
	_queue_text(horror_mgr.build_final_reveal(), TYPE_SPEED)
	await get_tree().create_timer(horror_mgr.build_final_reveal().size() * LINE_GAP + 3.0).timeout
